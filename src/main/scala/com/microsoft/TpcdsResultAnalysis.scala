...
object TpcdsResultAnalysis {
  ...
  private def analysis(spark: SparkSession, rawResult: String, csvResult: String) = {
    ...
    val results = result.select(explode(result("results")), col("version")).toDF("results", "version")

    results
      .withColumn("Name", col("results.name"))
      .withColumn("Time", (col("results.parsingTime") + col("results.analysisTime") + col("results.optimizationTime") + col("results.planningTime") + col("results.executionTime")) / 1000)
      .drop(col("results"))
      .groupBy("Name")
      .pivot("version")
      .sum("Time")
